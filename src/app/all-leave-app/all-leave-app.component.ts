  ...
  ngOnInit() {
    ...
    getDataForTable(){
      ...
      _.forEach(this.leaveApp, leave=>{
        var subData = [];
        _.forEach(this.dataObject.columns, data=>{
          console.log(leave[data.title]);
          if(data.title=='dev'){
            subData.push(leave[data.title]?leave[data.title].profilePhoto:"");
          }else{
            subData.push(leave[data.title]?leave[data.title]:"");
          }
        });
        data.push(subData);
      })
